	  ...
	  if(dc == 0)
		{
		  ...
		  //pol2 270
		  //getDetection(creal(out_p0[i]),cimag(out_p0[i]),cimag(out_p1[i]),-creal(out_p1[i]),dets,dstat);
		  // pol1 90, pol2 270
		  //getDetection(-cimag(out_p0[i]),creal(out_p0[i]),cimag(out_p1[i]),-creal(out_p1[i]),dets,dstat);
		  // pol2 180
		  //getDetection(creal(out_p0[i]),cimag(out_p0[i]),-creal(out_p1[i]),-cimag(out_p1[i]),dets,dstat); 
			  for(j=0;j<npol;j++)
				det[k][j]+=dets[j];
